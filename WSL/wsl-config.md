...
|:-----------|:-------------------------------|:-------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Включено    | boolean                        | true         | Значение `true` обеспечивает автоматическое подключение несъемных дисков (например, `C:/` или `D:/`) DrvFs в `/mnt`.  `false`означает, что диски не будут подключены автоматически, но их можно подключать вручную или через `fstab` .                                                                                                             |
...
**Примечание.** Маски разрешений помещаются в логическую операцию или перед применением к файлам или каталогам. 
...
| ключ | значение | default | HDInsight|
|:----|:----|:----|:----|
| Включено | boolean | `true` | Установка этого ключа определяет, будет ли WSL поддерживать запуск процессов Windows. |
