...
|:----|:----|:----|:----|
| default | строка | Начальное имя пользователя, созданное при первом запуске | Задание этого параметра указывает, какой пользователь будет запускать, как при первом запуске сеанса WSL. |
...
| Память | размер; | 80% общего объема памяти в Windows * | Объем памяти, назначаемый виртуальной машине WSL 2. |
| обработчики | число | Одинаковое число процессоров в Windows | Количество процессоров, назначаемых виртуальной машине WSL 2. |
| локалхостфорвардинг | Логическое | `true` | Логическое значение, указывающее, должны ли порты, привязанные к подстановочным знакам или localhost на виртуальной машине WSL 2, подключаться с узла через порт localhost:. |
| кернелкоммандлине | строка | Пустое | Дополнительные аргументы командной строки ядра. |
...
| Файл подкачки | строка | %усерпрофиле%\аппдата\локал\темп\свап.вхдкс | Абсолютный путь Windows к виртуальному жесткому диску для переключения. |

* Примечание. это значение true для Windows Build 19041 и может отличаться в сборках Windows в программе "предварительные оценки"
